...
public sealed partial class GitRepositoryClient
{
    ...
    private const string WorktreeCaptureScript = """
        ...
        temporary=$7
        if [ -z "$temporary" ]; then temporary=$(mktemp -d "${TMPDIR:-/tmp}/asura-git-capture.XXXXXX"); fi
        ...
        WorktreeCapture? expected = null)
    {
        ...
        if (writeObjects)
        {
            var allocation = await ExecuteIsolatedCommandAsync(repository, SealedGitEnvironment,
                "/bin/sh", ["-c", "umask 077; mktemp -d /tmp/asura-git-capture.XXXXXXXXXXXX"],
            ...
            const string prefix = "/tmp/asura-git-capture.";
        ...
        try
        {
            var result = await ExecuteIsolatedCommandAsync(
                ...
                "/bin/sh",
                ["-c", WorktreeCaptureScript, "asura-worktree-capture", repository.WorkingTreeRoot, path,
        ...
        finally
        {
            if (temporary.Length > 0)
            {
                _ = await ExecuteIsolatedCommandAsync(repository, SealedGitEnvironment,
                    "/bin/sh", ["-c", "rm -rf -- \"$1\"", "asura-capture-cleanup", temporary],
