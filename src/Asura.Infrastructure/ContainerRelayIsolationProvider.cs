...
public sealed class ContainerRelayIsolationProvider : IWorkspaceConnectionServiceProvider
{
    ...
    private const string Helper = "/opt/asura/asura-relay-gateway";
    ...
    internal const string ImageRecipe = """
        ...
        RUN mkdir -p /opt/asura /home/asura && tar --no-same-owner -xzf /tmp/payload.tar.gz -C /opt/asura && cd /opt/asura && sha256sum -c MANIFEST.sha256 && chmod 755 asura-relay-gateway && chown 1000:1000 /home/asura && rm /tmp/payload.tar.gz
        ENV HOME=/home/asura
        WORKDIR /home/asura
        CMD ["sleep", "infinity"]
        """;
        ...
        WorkspaceIsolationPrepareRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await CheckedAsync(engine, ["create", "--name", name, "--label", "com.asura.relay=" + id.ToString("N"),
                ...
                "--tmpfs", "/tmp:rw,nosuid,nodev,mode=1777,size=256m", "--tmpfs", "/home/asura:rw,exec,nosuid,nodev,mode=0700,size=768m",
            ...
            await CheckedAsync(engine, ["exec", "--user", "0", name, "/bin/chown", "1000:1000", "/home/asura"], cancellationToken).ConfigureAwait(false);
    ...
    public WorkspaceIsolationResult<WorkspaceProcessLaunch> CreateExecLaunch(WorkspaceIsolationBinding binding, WorkspaceIsolationProcessRequest request)
    {
        ...
        List<string> arguments = ["exec", "--interactive", "--user", "1000:1000", "--workdir", "/home/asura"];
