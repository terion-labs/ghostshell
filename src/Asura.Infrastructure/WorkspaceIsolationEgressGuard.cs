...
{
    public const string Namespace = "asura-vpn";
...
public sealed class WorkspaceIsolationEgressGuard : IWorkspaceIsolationEgressGuard
{
    ...
    private const string ArmScript = RootFunction + """
        ...
        '
        if as_root nft list table inet asura_guard >/dev/null 2>&1; then
            {
                printf '%s\n' 'delete table inet asura_guard'
                printf '%s\n' "$guard_rules"
            } | as_root nft -f -
        else
            printf '%s\n' "$guard_rules" | as_root nft -f -
        fi
        as_root nft list chain inet asura_guard mark_output >/dev/null
        as_root nft list chain inet asura_guard block_output \
            | grep -Eq 'policy drop'
        guard_installed=1
        proxy_state="$state/proxy"
        proxy_uid=$(id -u asura-net 2>/dev/null || true)
        ...
        as_root nft delete table ip asura_proxy >/dev/null 2>&1 || true
        if as_root test -f "$proxy_state/resolv.conf"; then
            as_root sh -c 'cat "$1" > /etc/resolv.conf' \
                asura-proxy-resolver "$proxy_state/resolv.conf"
        fi
        as_root rm -rf -- "$proxy_state"
        if as_root ip netns list | grep -Eq '^asura-vpn([[:space:]]|$)'; then
        ...
        as_root nft delete table ip asura_vpn_nat >/dev/null 2>&1 || true
        as_root nft delete table ip6 asura_vpn_nat >/dev/null 2>&1 || true
        printf '%s\n' '
            table ip asura_vpn_nat {
                chain postrouting {
    ...
    private const string ProxyArmScript = RootFunction + """
        ...
        id asura-net >/dev/null 2>&1 || exit 69
        if [ "$(id -u)" -ne 0 ]; then
            command -v sudo >/dev/null 2>&1 || exit 77
            sudo -n true >/dev/null 2>&1 || exit 77
        fi
        proxy_uid=$(id -u asura-net)
        proxy_gid=$(id -g asura-net)
        guard_rules=$(printf '
            table inet asura_guard {
                chain block_output {
                    type filter hook output priority filter; policy drop;
        ...
        proxy_ip=$(as_root runuser -u asura-net -- getent ahostsv4 "$proxy_host" \
            | awk '{ print $1; exit }')
        ...
        as_root sh -c 'umask 077; mkdir -p "$1"; cat > "$2"' \
            asura-proxy "$proxy_state" "$proxy_config"
        as_root sed -i "s/ASURA_PROXY_IP/$proxy_ip/g" "$proxy_config"
        ...
        as_root runuser -u asura-net -- redsocks \
        ...
        as_root nft list chain ip asura_proxy redirect_output \
    ...
    private const string DisarmScript = RootFunction + """
        ...
        if as_root nft list table ip asura_proxy >/dev/null 2>&1; then
            try_cleanup as_root nft delete table ip asura_proxy
        fi
        if as_root nft list table ip asura_proxy >/dev/null 2>&1; then
            cleanup_failed=1
        fi
        ...
        if as_root nft list table ip asura_vpn_nat >/dev/null 2>&1; then
            try_cleanup as_root nft delete table ip asura_vpn_nat
        fi
        if as_root nft list table ip6 asura_vpn_nat >/dev/null 2>&1; then
            try_cleanup as_root nft delete table ip6 asura_vpn_nat
        fi
        if as_root nft list table ip asura_vpn_nat >/dev/null 2>&1; then
            cleanup_failed=1
        fi
        ...
        if [ "$cleanup_failed" -ne 0 ]; then exit 70; fi
        as_root nft delete table inet asura_guard >/dev/null 2>&1
        if as_root nft list table inet asura_guard >/dev/null 2>&1; then exit 70; fi
        exit 0
        """;
        ...
        CancellationToken cancellationToken)
    {
        var launch = _isolationProvider.CreateExecLaunch(
            ...
            new WorkspaceIsolationProcessRequest(
                ConnectionKind.Local,
                "/bin/sh",
                ["-c", script, "asura-network-guard", .. arguments]));
        if (launch is WorkspaceIsolationResult<WorkspaceProcessLaunch>.Failure launchFailure)
        {
