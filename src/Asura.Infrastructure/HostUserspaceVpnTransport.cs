...
internal sealed class HostUserspaceVpnTransport : IHostUserspaceVpnTransport
{
            ...
            CancellationToken cancellationToken)
    {
        ...
        var engine = openVpn is null ? null : _executableLocator.Find("asura-openvpn-engine");
        if (executable is null || (openVpn is not null && engine is null))
        {
            return RuntimeMissing(
                "packet_vpn_host_runtime_missing",
                $"The bundled {name} engine is missing or invalid. Reinstall or update Asura.");
        }
            ...
            CancellationToken cancellationToken)
    {
        ...
        if (openconnect is null || gateway is null)
        {
            return RuntimeMissing(
                ...
                "The bundled Cisco AnyConnect connection engine is missing or invalid. Reinstall or update Asura to restore the connection engine payload.");
        }
            ...
            CancellationToken cancellationToken)
    {
        ...
        if (OperatingSystem.IsWindows())
        {
            return RuntimeMissing(
                ...
                "A private app-scoped tailscaled control socket is not yet available on Windows in this build. Asura will not reuse the system-wide Tailscale service.");
        }
        ...
        if (tailscaled is null || tailscale is null)
        {
            return RuntimeMissing(
                ...
                "The bundled Tailscale connection engine is missing or invalid. Reinstall or update Asura to restore the connection engine payload.");
        }
                ...
                var (stableCode, reason) = diagnostic switch
                {
                    ...
                    _ when diagnostic.Contains("it is a local IP address to this machine", StringComparison.Ordinal) =>
                        ("tailscale_exit_node_is_self",
                            "The selected address belongs to Asura's private Tailscale client itself. Select a different Tailscale device as the exit node."),
    ...
    {
        private const string Prefix = "asura-vpn-";
