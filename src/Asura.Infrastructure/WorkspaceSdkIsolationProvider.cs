...
public sealed partial class WorkspaceSdkIsolationProvider : IWorkspaceConnectionServiceProvider
{
    ...
    private const string GuestHome = "/home/asura";
    ...
    private WorkspaceSdkIsolationProvider(string executable, string? stateRoot, bool serviceIsolate)
        : this(
            executable,
            stateRoot ?? Path.Combine(AsuraDataPaths.CreateDefault().DataDirectory, "sdk-workspaces"),
        ...
        switch (request.ConnectionKind)
        {
            ...
            case ConnectionKind.Ssh when string.Equals(Path.GetFileName(request.HostExecutable), "ssh", StringComparison.Ordinal):
                ...
                arguments = ["/bin/sh", "-c", AppleContainerWorkspaceIsolationProvider.SshBootstrapScript,
                    "asura-ssh", "/usr/bin/ssh", knownHosts, trust, .. sshArguments];
                break;
            default:
    ...
    private Task<string> PrepareBootAssetsAsync(IProgress<WorkspaceIsolationProgress>? progress, CancellationToken cancellationToken)
    {
        ...
        var assets = new WorkspaceBootAssets(Path.Combine(assetDirectory, "boot-assets.json"), Path.Combine(_stateRoot, "boot"),
            new Uri($"https://github.com/terion-labs/asura/releases/download/v{version}/{WorkspaceBootAssets.ArchiveName}"));
        ...
        return assets.EnsureAsync(progress, cancellationToken,
            localArchive: Environment.GetEnvironmentVariable("ASURA_WORKSPACE_BOOT_ARCHIVE"));
    }
    ...
    private async ValueTask<IReadOnlyList<uint>> ProvisionUserAsync(string socket, CancellationToken cancellationToken)
    {
        var request = new WorkspaceSdkExecRequest(
            ["/bin/sh", "-c", AppleContainerWorkspaceIsolationProvider.GuestProvisioningScript, "asura-provision", "asura",
