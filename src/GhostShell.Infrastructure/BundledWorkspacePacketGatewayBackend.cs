...
internal sealed class AppleContainerGuestPacketRouterLauncher : IWorkspaceGuestPacketRouterLauncher
{
    ...
    private const string GuestPidFilePath = "/var/lib/ghostshell/network.pid";
...
{
    private const string HostHelperBaseName = "ghostshell-workspace-gateway";
    private static readonly TimeSpan ReadinessTimeout = TimeSpan.FromSeconds(30);
            ...
            CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(_hostHelperExecutable))
        {
            return Fail(
                ...
                "The workspace network helper is missing from this GhostSHELL installation.",
                retryable: false);
        }
            ...
            else
            {
                ...
                if (dnsServers.Count == 0)
                {
                    return Fail(
                        ...
                        "GhostSHELL could not read the host DNS settings for the isolated workspace.",
                        retryable: true);
                }
            }
    ...
    internal static string? ResolveHostHelperExecutable(IConnectionExecutableLocator locator)
    {
        ...
        return File.Exists(bundled)
            ? bundled
            : locator.Find(artifactName) ?? locator.Find("ghostshell-workspace-network");
    }
    ...
    private sealed class ProcessBackedSession : IHostWorkspacePacketGatewayBackendSession
    {
            ...
            IWorkspaceGatewayProcess process)
        {
            ...
            {
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=provider-channel-closed", StringComparison.Ordinal) =>
                    ("provider-channel-closed", "The VPN-to-workspace packet channel closed."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=provider-channel-failed", StringComparison.Ordinal) =>
                    ("provider-channel-failed", "The VPN-to-workspace packet channel failed."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=provider-handshake-failed", StringComparison.Ordinal) =>
                    ("provider-handshake-failed", "The VPN packet channel could not be established."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=workspace-nic-congestion", StringComparison.Ordinal) =>
                    ("workspace-nic-congestion", "The host workspace network interface ran out of buffer space."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=workspace-nic-failed", StringComparison.Ordinal) =>
                    ("workspace-nic-failed", "The host workspace network interface failed."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=protocol-authentication", StringComparison.Ordinal) =>
                    ("protocol-authentication", "The packet channel failed authentication."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=protocol-malformed", StringComparison.Ordinal) =>
                    ("protocol-malformed", "The packet channel received a malformed packet."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=protocol-sequence", StringComparison.Ordinal) =>
                    ("protocol-sequence", "The packet channel lost packet ordering."),
                _ when diagnostic.Contains("GHOSTSHELL_GATEWAY_EXIT_REASON=runtime-panic", StringComparison.Ordinal) =>
