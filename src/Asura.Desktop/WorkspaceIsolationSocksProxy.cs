    ...
    IWorkspaceNetworkConnector
{
        ...
        string? browserProfileRouteIdentity = null)
    {
        ...
        {
            IsBackground = true,
            Name = "Asura workspace browser proxy",
        };
        _acceptThread.Start();
    }
    ...
    private void AcceptLoop()
    {
        while (!_lifetime.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    IsBackground = true,
                    Name = "Asura workspace browser connection",
                };
    ...
    private async Task ServeAsync(TcpClient client, CancellationToken cancellationToken)
    {
                   ...
                   route.CancellationToken))
        {
            ...
            {
                IsBackground = true,
                Name = "Asura workspace browser upload",
            };
            var downstream = new Thread(() => Copy(process.StandardOutput.BaseStream, stream))
            {
                IsBackground = true,
                Name = "Asura workspace browser download",
            };
            upstream.Start();
            downstream.Start();
            upstream.Join();
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                IsBackground = true,
                Name = "Asura workspace proxy upload",
            };
            var download = new Thread(() => Copy(upstream, downstream))
            {
                IsBackground = true,
                Name = "Asura workspace proxy download",
            };
            upload.Start();
            download.Start();
            upload.Join();
        ...
        CancellationToken cancellationToken)
    {
        var planned = await _commandRuntime.PlanDuplexCommandAsync(
            ...
            [
                "-c",
                "if command -v nc >/dev/null 2>&1; then exec nc \"$1\" \"$2\"; elif [ -x /bin/bash ]; then exec /bin/bash -c 'exec 3<>\"/dev/tcp/$1/$2\"; cat <&3 & cat >&3; wait' asura-tcp \"$1\" \"$2\"; else exit 127; fi",
                "asura-tcp",
                host,
