...
{
    internal const string ArchiveName = "Asura-workspace-backend-arm64.tar.gz";
    ...
    internal const string InstallScript = """
        set -eu
        umask 077
        root=/home/asura/.cache/asura/backend
        mkdir -p "$root"
        target="$root/$1"
        ...
        chmod 700 "$staging/files/Asura.Backend"
        if [ -e "$target" ]; then
    ...
    internal async Task<DatabaseWorkspaceOperationLaunch> PlanAsync(string capability, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (_executable is null)
            {
                ...
                var start = await PlanCommandAsync("/bin/sh", ["-c", InstallScript, "asura-backend-install", archive.Hash, installationId], cancellationToken).ConfigureAwait(false);
                ...
                _executable = $"/home/asura/.cache/asura/backend/{archive.Hash}/Asura.Backend";
            }
    ...
    private async Task CleanupInstallationAsync(string installationId)
    {
        ...
        var start = await PlanCommandAsync("/bin/sh",
            ["-c", "rm -rf -- \"/home/asura/.cache/asura/backend/install.$1\"", "asura-backend-cleanup", installationId],
        ...
        string? descriptorPath = null, string? cacheRoot = null)
    {
        ...
        return EnsureArchiveAsync(descriptorPath ?? descriptor,
            cacheRoot ?? Path.Combine(AsuraDataPaths.CreateDefault().DataDirectory, "workspace-backends"),
            new Uri($"https://github.com/terion-labs/asura/releases/download/v{version}/Asura-workspace-backend-{architecture}.tar.gz"),
            Environment.GetEnvironmentVariable(architecture is "arm64" ? "ASURA_WORKSPACE_BACKEND_ARCHIVE" : "ASURA_WORKSPACE_BACKEND_X64_ARCHIVE"), token);
    }
        ...
        Uri uri, string? localArchive, CancellationToken token, HttpMessageHandler? handler = null)
    {
        ...
        {
            throw new IOException("The workspace backend descriptor is missing. Build the workspace backend payload or reinstall Asura.");
        }
