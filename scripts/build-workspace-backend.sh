...
cp "${repository_dir}/LICENSE" "${staging}/legal/ASURA-LICENSE.txt"
...
cp "${repository_dir}/native/artifacts/linux-${architecture}/asura-workspace-gateway-linux-${gateway_arch}" "${staging}/asura-relay-gateway"
...
backend_version="$(python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); print(next(k.split("/",1)[1] for k,v in d["libraries"].items() if k.startswith("Asura.Backend/") and v["type"]=="project"))' "${staging}/Asura.Backend.deps.json")"
catalog="workspace-backend-managed-components.json"
[[ "${architecture}" != x64 ]] || catalog="workspace-backend-x64-managed-components.json"
"${dotnet}" run --project "${repository_dir}/tools/Asura.Packaging/Asura.Packaging.csproj" \
