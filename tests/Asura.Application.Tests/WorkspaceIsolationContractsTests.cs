...
public sealed class WorkspaceIsolationContractsTests
{
    ...
    public void Network_binding_canonicalizes_a_valid_ipv4_subnet()
    {
        ...
        Assert.Equal("/run/asura/network.sock", network.GuestSocketPath);
        Assert.Equal("/opt/asura/bin/workspace-gateway", network.GuestHelperPath);
    }
    ...
    private static string HostPath(string suffix) =>
        Path.Combine(Path.GetTempPath(), "asura-isolation-contracts", suffix);
}
