...
public sealed class GovernedGitRepositoryClientTests
{
    ...
    public async Task GovernedHttpsReadIsSealedAndPushIsWithheld()
    {
        ...
        await repository.RunGitAsync(
            ...
            "add",
            "origin",
            "https://example.invalid/asura.git");
        ...
        Assert.Contains(
            ...
            StringComparer.Ordinal);
        Assert.Contains(
            "http.https://example.invalid/asura.git.extraHeader=",
            remoteCommand.Arguments,
            StringComparer.Ordinal);
        Assert.Contains(
            ...
            StringComparer.Ordinal);
        Assert.Contains(
            "http.https://example.invalid/asura.git.followRedirects=false",
            remoteCommand.Arguments,
            StringComparer.Ordinal);
    ...
    public async Task Governed_https_read_preserves_the_workspace_proxy_after_scrubbing_untrusted_configuration()
    {
        await using var repository = await LocalRepository.CreateAsync();
        var initial = await repository.ReadStateAsync();
        await repository.RunGitAsync("remote", "add", "origin", "https://example.invalid/asura.git");
        ...
        : IConnectionCommandExecutor
    {
        ...
        {
            if (request.Arguments.Contains("asura-worktree-capture", StringComparer.Ordinal)
    ...
    private sealed class LocalRepository : IAsyncDisposable
    {
        ...
        public static async Task<LocalRepository> CreateAsync(bool withBareRemote = false)
        {
            ...
            var directory = Directory.CreateTempSubdirectory("asura-governed-git-");
            ...
            try
            {
                ...
                await fixture.RunGitAsync("config", "user.name", "Asura Test");
                await fixture.RunGitAsync("config", "user.email", "asura@example.invalid");
                await File.WriteAllTextAsync(
