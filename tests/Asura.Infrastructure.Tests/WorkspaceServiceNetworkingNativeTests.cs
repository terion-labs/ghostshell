...
public sealed class WorkspaceServiceNetworkingNativeTests(ITestOutputHelper output)
{
    ...
    {
        var archive = Environment.GetEnvironmentVariable("ASURA_TEST_RELAY_ARCHIVE")!;
        var gateway = Environment.GetEnvironmentVariable("ASURA_TEST_RELAY_GATEWAY")!;
    ...
    public async Task Service_guest_preserves_remote_loopback_and_private_names_through_host_SOCKS_gateway()
    {
        ...
        var directory = Directory.CreateTempSubdirectory("asura-service-network-native-");
            ...
            foreach (var host in new[] { "127.0.0.1", "127.12.34.56", "2001:db8::20", "::1", "localhost", "private-database.invalid", "198.51.100.20" })
            {
                ...
                catch
                {
                    var request = new WorkspaceSdkExecRequest(["/usr/bin/systemd-run", "--wait", "--pipe", "--collect", "--quiet",
                        ...
                        "ip -6 addr; ip -6 route show table all; nft list table inet asura_service; cat /proc/net/snmp6"],
    ...
    private sealed class ContainerRelayFactAttribute : FactAttribute
    {
        ...
        {
            if (Environment.GetEnvironmentVariable("ASURA_TEST_RELAY_ARCHIVE") is null
                || Environment.GetEnvironmentVariable("ASURA_TEST_RELAY_GATEWAY") is null)
            {
