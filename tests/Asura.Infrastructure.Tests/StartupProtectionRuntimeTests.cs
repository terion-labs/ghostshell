...
public sealed class StartupProtectionRuntimeTests : IDisposable
{
    private readonly string _root =
        Directory.CreateTempSubdirectory("asura-startup-protection").FullName;
    ...
    public async Task Enabling_protection_with_encryption_on_seals_the_keys_and_the_pin_releases_them()
    {
        ...
        var databasePath = Path.Combine(_root, "asura.db");
    ...
    private (ApplicationEncryptionRuntime Runtime, AsuraDatabase Database) ComposeEncryption(
        string databasePath)
    {
    ...
    private static async Task WriteProbeRowAsync(AsuraDatabase database)
    {
    ...
    private static async Task<string?> ReadProbeRowAsync(AsuraDatabase database)
    {
        await using var connection = await database.OpenConnectionAsync(CancellationToken.None);
        await using var command = connection.CreateCommand();
