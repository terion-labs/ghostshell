...
public sealed class WorkspaceGatewayProcessRunnerTests
{
    ...
    public async Task Exited_parent_with_inherited_output_does_not_block_disposal()
    {
        ...
        var directory = Directory.CreateTempSubdirectory("asura-orphan-test-").FullName;
    ...
    {
        private readonly string _directory = Directory.CreateTempSubdirectory("asura-runner-test-").FullName;
        private Process? _process;

        public WorkspaceGatewayProcessRequest Request(string script) => new(
            "/bin/sh",
            ["-c", "printf '%s' \"$$\" > \"$1\"; " + script, "asura-runner-test", Path.Combine(_directory, "pid")],
