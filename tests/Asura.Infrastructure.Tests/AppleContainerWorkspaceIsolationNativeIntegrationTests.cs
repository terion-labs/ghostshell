...
{
    private const string EnableVariable = "ASURA_RUN_APPLE_CONTAINER_NATIVE";
    private const string TerminalRuntimePathVariable = "ASURA_GHOSTTY_VT_PATH";
    ...
    public async Task Native_provider_reconfigures_mounts_without_losing_the_persistent_root()
    {
        ...
        var hostMount = Directory.CreateTempSubdirectory("asura-native-mount-");
        var replacementHostMount = Directory.CreateTempSubdirectory(
            "asura-native-replacement-mount-");
        ...
        const string marker = "asura-native-persistence-ok";
        ...
        try
        {
            ...
            await RunShellAsync(
                ...
                + "id -Gn | tr ' ' '\\n' | grep -Fx docker\n"
                + "sudo -n true\n"
                + "sudo install -o root -g docker -m 660 /dev/null /tmp/asura-docker-access\n"
                + "printf docker-access-ok > /tmp/asura-docker-access\n"
                + $"printf '%s' '{marker}' > \"$HOME/.asura-native-smoke\"\n"
                + "printf '#!/bin/sh\\nexit 0\\n' | sudo tee /usr/local/bin/asura-native-smoke >/dev/null\n"
                + "sudo chmod +x /usr/local/bin/asura-native-smoke\nexit\n",
                timeout.Token);
            ...
            await RunShellAsync(
                provider,
                second,
                $"test \"$(cat \"$HOME/.asura-native-smoke\")\" = '{marker}'\nexit\n",
                timeout.Token);
            ...
            await RunShellAsync(
                provider,
                recreated,
                "test ! -e \"$HOME/.asura-native-smoke\"\nexit\n",
                timeout.Token);
    ...
    [NativeAppleContainerFact("ASURA_TEST_WIREGUARD_CONFIG")]
    public Task Native_WireGuard_routes_guest_IP_DNS_and_https_then_closes_fail_closed() =>
        VerifyNativePacketGatewayAsync(Environment.GetEnvironmentVariable("ASURA_TEST_WIREGUARD_CONFIG"));

    [NativeAppleContainerFact("ASURA_TEST_OPENVPN_CONFIG")]
    public Task Native_OpenVPN_routes_guest_IP_DNS_and_https_then_closes_fail_closed() =>
        VerifyNativePacketGatewayAsync(Environment.GetEnvironmentVariable("ASURA_TEST_OPENVPN_CONFIG"), NetworkConnectionKind.OpenVpn);

    [NativeAppleContainerFact("ASURA_TEST_PROXY_URL_FILE")]
    public Task Native_authenticated_proxy_routes_guest_DNS_and_https_then_closes_fail_closed() =>
        VerifyNativePacketGatewayAsync(Environment.GetEnvironmentVariable("ASURA_TEST_PROXY_URL_FILE"), NetworkConnectionKind.Proxy);
    ...
    private static async Task VerifyNativePacketGatewayAsync(string? configurationPath, NetworkConnectionKind kind = NetworkConnectionKind.WireGuard)
    {
        ...
        else if (configurationPath is not null)
        {
            ...
            {
                var credentialPath = Environment.GetEnvironmentVariable("ASURA_TEST_OPENVPN_CREDENTIAL_FILE");
        ...
        try
        {
            ...
            var guestPid = (await RunCommandAsync(
                    ...
                    binding,
                    "/usr/bin/sudo",
                    ["-n", "cat", "/var/lib/asura/network.pid"],
                    timeout.Token))
                .Trim();
            Assert.True(
                int.TryParse(
            ...
            var guestStopped = await RunCommandAsync(
                ...
                [
                    ...
                    "/bin/sh",
                    "-c",
                    $"test ! -e /var/lib/asura/network.pid; "
                    + "test ! -e /var/lib/asura/network.sock; "
    ...
    private static string StagedWorkspaceGatewayHostPath()
    {
             ...
             directory = directory.Parent)
        {
            ...
            var helper = Path.Combine(
                ...
                "artifacts",
                "osx-arm64",
                "asura-workspace-gateway-darwin-arm64");
    ...
    {
        public string? Find(string executable) => executable == "asura-openvpn-engine"
