...
public sealed class ApplicationStorageIdentityTests
{
    ...
    {
        Assert.Equal("Asura Development", ApplicationStorageIdentity.DirectoryName);
        Assert.Equal("asura-development", ApplicationStorageIdentity.PosixDirectoryName);
        Assert.Equal("sh.asura.development", new SecretVaultFactoryOptions().ServiceName);
    }

    [Fact]
    public void Durable_browser_and_disposable_paths_use_the_same_build_namespace()
    {
        var data = AsuraDataPaths.CreateDefault();
        ...
        Assert.Equal(Path.Combine(data.DataDirectory, "asura.db"), data.DatabasePath);
