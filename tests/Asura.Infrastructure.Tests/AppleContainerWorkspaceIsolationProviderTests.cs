...
public sealed class AppleContainerWorkspaceIsolationProviderTests
{
    ...
    public async Task Prepare_creates_and_verifies_the_owned_host_only_network_when_absent()
    {
        ...
        Assert.Equal(
            [
                ...
                "--internal",
                "--label",
                $"io.asura.workspace={binding.ResourceName}",
                "--label",
                "io.asura.isolation-schema=3",
                networkName,
            ],
    ...
    public async Task Prepare_creates_each_configured_mount_without_forwarding_the_ssh_agent()
    {
        ...
        Assert.Contains(
            $"io.asura.base-image={AppleContainerWorkspaceIsolationProvider.DefaultImageReference}",
            create,
            StringComparer.Ordinal);
        Assert.Contains(
            create.Zip(create.Skip(1)),
    ...
    {
        var testDirectory = Directory.CreateTempSubdirectory("asura-packet-transport-");
        try
        {
    ...
    public async Task Prepare_rejects_a_missing_host_mount_before_invoking_the_runtime()
    {
        ...
        [
            new(
                Path.Combine(
                    Path.GetTempPath(),
                    "asura-missing-mount-166b7dcc29f34336b84b33e734e5faef"),
                "/workspace",
                isReadOnly: false),
        ];
    ...
    public async Task Prepare_allows_a_guest_only_persistent_environment()
    {
        ...
        Assert.Contains(
            ...
            pair => pair.First == "--workdir" && pair.Second == "/home/asura");
    }
    ...
    public void Local_launch_maps_the_host_shell_and_keeps_each_environment_value_structured()
    {
        ...
        var request = new WorkspaceIsolationProcessRequest(
            ...
            Path.Combine(HostHome, "projects", "asura"),
        ...
        string strictHostKeyChecking)
    {
        ...
        try
        {
            const string approvedKey = "asura-test ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAITest\n";
            ...
            var guestKnownHosts = Assert.Single(
                ...
                argument => argument.StartsWith(
                    "UserKnownHostsFile=/home/asura/.ssh/asura-known-hosts/",
                    StringComparison.Ordinal));
            Assert.Contains(
    ...
    public void A_host_credential_helper_is_not_mistaken_for_a_guest_executable()
    {
        ...
        var request = new WorkspaceIsolationProcessRequest(
            ConnectionKind.Ssh,
            "/Applications/Asura.app/Contents/MacOS/Asura.Desktop",
    ...
    private static string HostPath(string suffix) =>
        Path.Combine(Path.GetTempPath(), "asura-isolation-provider", suffix);
        ...
        string? packetGatewayHelperDirectory = null)
    {
        ...
        {
            ["io.asura.workspace"] = workspaceLabel ?? name,
            ["io.asura.isolation-schema"] = schemaVersion,
        };
        if (baseImageReference is not null)
        {
            labels["io.asura.base-image"] = baseImageReference;
        }
        ...
        return JsonSerializer.Serialize(new[]
        {
            new
            {
                configuration = new
                {
                    ...
                    {
                        ["io.asura.workspace"] = workspaceLabel ?? containerName,
                        ["io.asura.isolation-schema"] = "3",
                    },
                    mode,
                    name = networkName,
        ...
        string resourceName)
    {
        ...
        Assert.Equal("asura-provision", command.Arguments[5]);
        Assert.Equal("asura", command.Arguments[6]);
        Assert.Equal("1000", command.Arguments[7]);
        Assert.Equal("1000", command.Arguments[8]);
        Assert.Equal("/home/asura", command.Arguments[9]);
        ...
        string resourceName)
    {
        ...
        Assert.Contains("HOME=/home/asura", command.Arguments, StringComparer.Ordinal);
        Assert.Contains("/home/asura", command.Arguments, StringComparer.Ordinal);
