...
public sealed class MainWindowRuntimeGraphIntegrationTests
{
    ...
    public async Task PrefixEnterShortcutIsClaimedBeforeItsAsynchronousCommandReachesTheTerminal()
    {
        ...
        {
            var prefix = new KeyStroke("B", Asura.Core.KeyModifiers.Control);
            ...
            {
                window.UpdateLayout();
                var terminal = window.GetVisualDescendants().OfType<Asura.App.Controls.TerminalPresentationHost>().First();
                var leaked = 0;
    ...
    [Fact]
    public async Task BrowserPopupOpensARealAsuraTabWithTheSameRouteAndProfile()
    {
...
, Store(ssh), Store(docker)],
    ...
    {
        var gitId = new ConnectionId("connections.asura-repo");
    ...
    public async Task A_git_connection_referencing_a_saved_ssh_connection_resolves_it_live()
    {
        ...
        var gitId = new ConnectionId("connections.asura-repo-reference");
        var gitConnection = new ConnectionProfile(
            gitId,
            ConnectionProfile.CurrentSchemaVersion,
            "Asura repo",
