...
public sealed class BundledConnectionEngineExecutableLocatorTests : IDisposable
{
    private readonly DirectoryInfo _temporaryDirectory =
        Directory.CreateTempSubdirectory("ghostshell-bundled-engine-locator-");
    ...
    public void Find_prefers_a_bundled_connection_engine_over_path()
    {
        ...
        var bundled = CreateExecutable(bundle.FullName, "ghostshell-openvpn-engine");
        _ = CreateExecutable(path.FullName, "ghostshell-openvpn-engine");
        var locator = new BundledConnectionEngineExecutableLocator(
            new PathConnectionExecutableLocator(path.FullName, []),
            bundle.FullName,
            useBundle: true);

        Assert.Equal(bundled, locator.Find("ghostshell-openvpn-engine"));
    }

    [Theory]
    [InlineData("ghostshell-openvpn-engine")]
