...
public sealed partial class MacOsAppBundleBuilderTests : IDisposable
{
    ...
    private readonly string _temporaryDirectory = Path.Combine(
        Path.GetTempPath(),
        $"asura-package-tests-{Guid.NewGuid():N}");
    ...
    public void Builder_creates_the_exact_acceptance_bundle_without_modifying_publish_payload()
    {
        ...
        Assert.Equal("Asura.app", Path.GetFileName(result.DestinationPath));
        ...
        Assert.Equal("sh.asura", (string?)distributionManifest["packageId"]);
        ...
        {
            Assert.Equal(
                "Asura 1.2.3 managed-component evidence",
                document.RootElement.GetProperty("name").GetString());
            Assert.StartsWith(
                "https://asura.test/spdx/managed-components/1.2.3/",
                document.RootElement.GetProperty("documentNamespace").GetString(),
                StringComparison.Ordinal);
            Assert.Equal(
                "Tool: Asura.Packaging-1.0.0",
    ...
    public void Builder_rejects_a_fixed_first_party_version_in_the_reviewed_catalog()
    {
        ...
        var component = catalog["dependencies"]!
            ...
            .Single(node => string.Equals(
                ...
                "Asura.Core/${productVersion}",
                StringComparison.Ordinal));
        component["identity"] = "Asura.Core/1.2.3";
...
, "SkiaSharp.NativeAssets.macOS/3.119.3-preview.1.1", StringComparison.Ordinal));
    ...
    public void Builder_rejects_an_unresolved_selected_target_dependency()
    {
        var publish = CreatePublishPayload();
        var (path, dependencies) = ReadDependencies(publish);
        var root = SelectedTarget(dependencies)["Asura/1.2.3"]!.AsObject();
...
, StringComparison.Ordinal) ? " xmlns=\"urn:asura:test:wrong\""
            : string.Empty;
...
, "SkiaSharp.NativeAssets.macOS/3.119.3-preview.1.1", StringComparison.Ordinal));
    ...
    public void Builder_keeps_managed_evidence_out_of_the_native_aot_bundle()
    {
        ...
        File.Delete(Path.Combine(nativePublish, "Asura.deps.json"));
        File.Delete(Path.Combine(nativePublish, "Asura.runtimeconfig.json"));
        ...
        Assert.Empty(Directory.EnumerateFiles(
            ...
            "*.dll",
            SearchOption.TopDirectoryOnly));
        Assert.False(File.Exists(Path.Combine(executableDirectory, "Asura.deps.json")));
        Assert.False(File.Exists(Path.Combine(
            executableDirectory,
            "Asura.runtimeconfig.json")));
    }
    ...
    public void Builder_rejects_symbolic_links_in_the_publish_payload()
    {
        ...
        File.CreateSymbolicLink(
            ...
            Path.Combine(publish, "Asura.runtimeconfig.json"));
        var output = OutputPath();
    ...
    {
        var publish = CreatePublishPayload();
        var symbols = Path.Combine(publish, "Asura.dSYM");
    ...
    public void Builder_resolves_destination_ancestor_links_before_containment_checks()
    {
        ...
        var output = Path.Combine(alias, "Asura.app");
        ...
        Assert.False(Directory.Exists(Path.Combine(publish, "Asura.app")));
    }
    ...
    {
        var publish = CreatePublishPayload();
        var output = Path.Combine(publish, "Asura.app");
    ...
    public void Validated_publisher_fingerprints_and_moves_a_private_candidate()
    {
        ...
        var candidateParent = Path.Combine(
            outputParent,
            $".asura-package.{Guid.NewGuid():N}");
        Directory.CreateDirectory(candidateParent);
        var candidate = Path.Combine(candidateParent, "Asura.app");
        var publish = CreatePublishPayload();
        _ = new MacOsAppBundleBuilder().Build(
            Request(publish, candidate));
        var output = Path.Combine(outputParent, "Asura.app");
    ...
    private string OutputPath()
    {
        ...
        return Path.Combine(parent, "Asura.app");
    }
    ...
    {
        var path = Path.Combine(publishDirectory, "Asura.deps.json");
        return (
            path,
    ...
    private string CreatePublishPayload()
    {
        ...
        WriteFile(directory, "Asura", "executable");
        WriteFile(directory, "Asura.runtimeconfig.json", "{}");
    ...
    private void CreateEvidenceInputs(string publishDirectory)
    {
        ...
        rootTarget["dependencies"] = libraries.Keys
            .Where(identity => !string.Equals(identity, "Asura/1.2.3", StringComparison.Ordinal))
            .ToDictionary(
        ...
        File.WriteAllText(
            ...
            JsonSerializer.Serialize(
                new Dictionary<string, object?>(StringComparer.Ordinal)
                {
                    ...
                    ["namespaceBase"] =
                        "https://asura.test/spdx/managed-components/${productVersion}",
    ...
    private static ProductIdentityInputs CreateProductIdentityInputs(string fixtureDirectory)
    {
        ...
        File.WriteAllText(
            ...
            JsonSerializer.Serialize(new
            {
                format = "asura-macos-product-identity-v1",
                platform = "macos",
                displayName = "Asura",
                bundleName = "Asura",
                executableName = "Asura",
                bundleIdentifier = "sh.asura",
                iconName = "Asura",
                artwork = new
                {
                    source = "Original first-party Asura artwork",
                    owner = "Terion Labs",
                    license = "MIT",
                ...
                {
                    status = "approved",
                    approvedBy = "terion-labs/asura maintainer",
                    approvedAt = "2026-08-25",
                    evidence = "https://github.com/terion-labs/asura/issues/42",
                },
                ...
                files = new[]
                {
                    IdentityFile(
                        ...
                        "assets/macos/Asura.icon/icon.json",
                        documentPath),
                    IdentityFile(
                        "source-artwork",
                        "assets/macos/Asura.icon/Assets/logo.svg",
                        artworkPath),
                    IdentityFile(
                        "icns-fallback",
                        "assets/macos/Asura.icns",
                        fallbackPath),
                },
            }, new JsonSerializerOptions { WriteIndented = true }));
        ...
        NativeTerminalTestEvidence nativeEvidence)
    {
        ...
        {
            ["ASURA-LICENSE.txt"] = sourceFiles["LICENSE"],
