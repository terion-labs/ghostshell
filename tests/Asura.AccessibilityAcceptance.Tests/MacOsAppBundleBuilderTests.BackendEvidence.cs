...
using PackagingProgram = Asura.Packaging.Program;
...
public sealed partial class MacOsAppBundleBuilderTests
{
    ...
    public void BackendEvidenceUsesItsOwnExactLinuxClosureAndSpdxRoot()
    {
        ...
        Assert.Contains(spdx.RootElement.GetProperty("packages").EnumerateArray(), package => package.GetProperty("name").GetString() == "Asura.Backend");
        Assert.DoesNotContain(spdx.RootElement.GetProperty("packages").EnumerateArray(), package => package.GetProperty("name").GetString() == "Asura");
    }
    ...
    {
        var fixture = CreateBackendEvidenceFixture();
        var manifestPath = Path.Combine(fixture.Publish, "Asura.Backend.deps.json");
        ...
        switch (mutation)
        {
            ...
            case "root": manifest["targets"]![".NETCoreApp,Version=v10.0/linux-arm64"]!["Asura.Backend/1.2.3"]!["dependencies"] = new JsonObject(); break;
    ...
    private (string Publish, string Catalog, string Packages) CreateBackendEvidenceFixture()
    {
        ...
        foreach (var identity in libraries.Select(pair => pair.Key).Where(identity => identity != "Asura.Backend/1.2.3"))
        {
            var parts = identity.Split('/');
            dependencies[parts[0]] = parts[1];
        }
        target["Asura.Backend/1.2.3"]!["dependencies"] = dependencies;
        ...
        File.WriteAllText(Path.Combine(publish, "Asura.Backend.deps.json"), manifest.ToJsonString());
        var catalog = Path.Combine(publish, "catalog.json");
        File.WriteAllText(catalog, new JsonObject
        {
            ["schemaVersion"] = 2,
            ["documentName"] = "Asura ${productVersion} managed-component evidence",
            ["documentCreatedUtc"] = "2026-08-25T00:00:00Z",
            ["namespaceBase"] = "https://asura.sh/spdx/managed-components/${productVersion}",
