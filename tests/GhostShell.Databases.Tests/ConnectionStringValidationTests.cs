...
{
    [Theory]
    [InlineData("sqlite", "Data Source=/nonexistent/ghostshell-export.db;Mode=ReadOnly")]
    ...
    [InlineData("duckdb", "Data Source=/nonexistent/ghostshell-export.duckdb")]
    ...
    public async Task Supported_provider_options_validate_without_opening_connections(string driver, string target)
    {
        ...
        Assert.False(File.Exists("/nonexistent/ghostshell-export.db"));
        Assert.False(File.Exists("/nonexistent/ghostshell-export.duckdb"));
    }
