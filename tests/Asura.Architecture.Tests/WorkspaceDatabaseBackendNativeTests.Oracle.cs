...
public sealed partial class WorkspaceDatabaseBackendNativeTests
{
    ...
    public async Task Service_guest_Oracle_keeps_private_DNS_redirects_and_TLS_hostname_validation()
    {
        ...
        var directory = Directory.CreateTempSubdirectory("asura-service-oracle-");
        ...
        try
        {
            ...
            Assert.Equal("fixture-written", await RunShellAsync(commands, """
                set -eu
                printf '%s' "$1" | base64 -d > /tmp/asura-oracle-fixture.der
                openssl x509 -inform DER -in /tmp/asura-oracle-fixture.der -out /tmp/asura-oracle-fixture.crt
                printf fixture-written
                """, [certificate], deadline.Token));
            var trustRequest = new WorkspaceSdkExecRequest(["/bin/sh", "-c", """
                set -eu
                install -m 644 /tmp/asura-oracle-fixture.crt /usr/local/share/ca-certificates/asura-oracle-fixture.crt
