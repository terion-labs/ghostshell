...
public sealed class SqlLanguageWorkerPackagingTests
{
    ...
    public void DesktopPublishPreservesTheRidScopedWorkerLocation()
    {
        ...
        var worker = Assert.Single(
            ...
            element => string.Equals(
                ...
                "$(AsuraSqlLanguageWorkerPath)",
                StringComparison.Ordinal));

        Assert.Equal(
            "runtimes/$(AsuraEffectiveRuntimeIdentifier)/native/$(AsuraSqlLanguageWorkerName)",
        ...
        var linkedPayload = project.Descendants("Content")
            ...
            .ToArray();
        Assert.Contains(
            "runtimes/$(AsuraEffectiveRuntimeIdentifier)/native/THIRD-PARTY-NOTICES.md",
            linkedPayload, StringComparer.Ordinal);
        Assert.Contains(
            "runtimes/$(AsuraEffectiveRuntimeIdentifier)/native/runtime-dependencies.txt",
            linkedPayload, StringComparer.Ordinal);
        Assert.Contains(
            "runtimes/$(AsuraEffectiveRuntimeIdentifier)/native/build-receipt.json",
            linkedPayload, StringComparer.Ordinal);
    }
    ...
    public void ReleasePublishFailsClosedWithoutTheCompleteSupportedRidPayload()
    {
        ...
        Assert.Equal(
            "false",
            project.Descendants("AsuraSqlLanguageRequired").Single().Value);
        ...
        var requiredFiles = project.Descendants("AsuraSqlLanguageRequiredFile")
            .Select(element => (string?)element.Attribute("Include"))
            .Where(include => include is not null)
            .ToArray();
        Assert.Contains("$(AsuraSqlLanguageWorkerPath)", requiredFiles, StringComparer.Ordinal);
        Assert.Contains(
            "$(AsuraSqlLanguageArtifactDirectory)/THIRD-PARTY-NOTICES.md",
            requiredFiles, StringComparer.Ordinal);
        Assert.Contains(
            "$(AsuraSqlLanguageArtifactDirectory)/runtime-dependencies.txt",
            requiredFiles, StringComparer.Ordinal);
        Assert.Contains(
            "$(AsuraSqlLanguageArtifactDirectory)/build-receipt.json",
            requiredFiles, StringComparer.Ordinal);
    }
    ...
    public void MacPackageRequiresTheWorkerBeforeAndAfterBundling()
    {
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "candidate}/Contents/MacOS/runtimes/osx-arm64/native/asura-sql-language",
            script,
            StringComparison.Ordinal);
        Assert.Contains(
    ...
    public void Dormant_database_conformance_preserves_the_native_worker_contract()
    {
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "ASURA_SQL_LANGUAGE_WORKER: ${{ github.workspace }}/native/artifacts/linux-x64/asura-sql-language",
            workflow,
            StringComparison.Ordinal);
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "ASURA_SQL_LANGUAGE_WORKER is required when ASURA_RUN_SQL_LANGUAGE_NATIVE=1.",
            wrapper,
            StringComparison.Ordinal);
    ...
    public void SqlWorkerRoutesLocalMavenOutputsOutsideASealedSource()
    {
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "<directory>${asura.build.directory}</directory>",
            project,
            StringComparison.Ordinal);
        Assert.DoesNotContain(
    ...
    public void EverySupportedSelfContainedPackageUsesTheServicedRuntimePack()
    {
        var project = XDocument.Load(Path.Combine(
            ...
            "Asura.Desktop",
            "Asura.Desktop.csproj"));
        Assert.Equal("10.0.11", project.Descendants("AsuraRuntimePackVersion").Single().Value);
        Assert.Contains(
            "$(AsuraRuntimePackVersion)",
            project.Descendants("AsuraRuntimePackDirectory").Single().Value,
            StringComparison.Ordinal);
        ...
        Assert.Contains(
            "%(AsuraSelectedRuntimePack.NuGetPackageVersion)' != '$(AsuraRuntimePackVersion)",
        ...
        Assert.Contains(
            "$(MSBuildThisFileDirectory)src/Asura.*/*.csproj",
            restoreGuard.ToString(),
            StringComparison.Ordinal);
        Assert.Contains("AsuraExpectedRuntimeLock", restoreGuard.ToString(), StringComparison.Ordinal);
        ...
        Assert.Contains("AsuraRequiredRuntimeLock", restoreGuard.ToString(), StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            restoreGuard.Descendants("Error"),
            error => error.Attribute("Condition")?.Value.Contains(
                "@(AsuraSelectedRuntimeLock)' != '@(AsuraExpectedRuntimeLock)",
                StringComparison.Ordinal) == true);
        Assert.DoesNotContain(
        ...
        var supportedRidCondition = project.Descendants("AsuraSqlLanguageSupported")
            .Single()
            .Attribute("Condition")!
            .Value;
        foreach (var rid in supportedRids)
        ...
        var firstPartyProjects = managedCatalog.RootElement
            ...
            .Where(dependency =>
                ...
                && (identity.StartsWith("Asura.", StringComparison.Ordinal)
                    || identity.StartsWith("Asura/", StringComparison.Ordinal)))
            .ToArray();
    ...
    public void NativeAotProbeCompilesTheExactProductionClientSources()
    {
        var project = XDocument.Load(Path.Combine(
            ...
            "tools",
            "Asura.SqlLanguageAotProbe",
            "Asura.SqlLanguageAotProbe.csproj"));
        Assert.Equal(
            "true",
        ...
        var linkedSources = project.Descendants("Compile")
            ...
            .ToArray();
        Assert.Contains(
            "../../src/Asura.Infrastructure/CalciteSqlLanguageService.cs",
            linkedSources, StringComparer.Ordinal);
        Assert.Contains(
            "../../src/Asura.Infrastructure/CalciteSqlLanguageSession.cs",
            linkedSources, StringComparer.Ordinal);
        Assert.Contains(
            "../../src/Asura.Infrastructure/SqlLanguageWorkerProtocol.cs",
            linkedSources, StringComparer.Ordinal);
        Assert.Contains(
            "../../src/Asura.Infrastructure/UnavailableSqlLanguageSession.cs",
            linkedSources, StringComparer.Ordinal);
    }
