...
public sealed class DatabaseRecoveryExportTests
{
    ...
    {
        var directory = Directory.CreateTempSubdirectory("asura-database-export-").FullName;
        try
        {
            await using var database = new AsuraDatabase(new(Path.Combine(directory, "test.db")), TimeProvider.System);
    ...
    {
        var directory = Directory.CreateTempSubdirectory("asura-database-export-invalid-").FullName;
        try
        {
            await using var database = new AsuraDatabase(new(Path.Combine(directory, "test.db")), TimeProvider.System);
            await using var relational = new DatabasePanelClient();
    ...
    private static async Task<string> ReadPayloadAsync(AsuraDatabase database)
    {
