...
public sealed class DatabaseOperationWorkerTests(ITestOutputHelper output)
{
    ...
    public async Task SnapshotTransferCannotExecuteSqlBeforeCompleteImageAndExecuteAcknowledgement(bool completeImage)
    {
        ...
        var target = Asura.Databases.SqliteInMemoryDatabases.Register(image);
        ...
        try
        {
            ...
            Asura.Databases.SqliteInMemoryDatabases.Unregister(target);
    ...
    public async Task RemoteSqlitePreviewImageIsCompleteAndReadOnlyInActualWorker(bool maximumImage)
    {
        ...
        finally { Asura.Databases.SqliteInMemoryDatabases.Unregister(target); }
    }
    ...
    {
        private readonly DirectoryInfo _directory = Directory.CreateTempSubdirectory("asura-operation-test-");
