...
{
    [Theory]
    [InlineData("api.github.com/repos/terion-labs/asura/releases")]
    [InlineData("releases/latest/download/Asura-macOS")]
    [InlineData("Sparkle")]
    [InlineData("appcast")]
    ...
    public void Velopack_dependency_is_isolated_to_the_updates_boundary()
    {
        ...
        Assert.Equal(
            ["src/Asura.Updates/Asura.Updates.csproj"],
    ...
    public void Macos_release_packages_a_speed_optimized_native_aot_executable()
    {
        ...
        Assert.Contains("-p:AsuraMacReleaseNativeAot=true", packageScript, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("<string>Asura.icns</string>", infoPlist, StringComparison.Ordinal);
    ...
    public void Macos_product_identity_is_reviewed_hashed_and_compiled_by_xcode()
    {
        ...
        Assert.Contains("--app-icon Asura", iconCompiler, StringComparison.Ordinal);
    ...
    public void Tag_release_publishes_stable_latest_download_assets()
    {
        ...
        Assert.Contains("archive=\"Asura-macOS-arm64.zip\"", workflow, StringComparison.Ordinal);
        Assert.Contains("distribution/Asura-macOS-arm64.zip.sha256", workflow, StringComparison.Ordinal);
        Assert.DoesNotContain("Asura-macOS-arm64-${RELEASE_VERSION}.zip", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("sh.asura-${RELEASE_VERSION}-osx-arm64-stable-full.nupkg", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("name: asura-macos-arm64", workflow, StringComparison.Ordinal);
    ...
    public void Macos_packaging_keeps_vendored_project_versions_independent()
    {
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "<Version>$(AsuraProductVersion)</Version>",
            buildProperties,
            StringComparison.Ordinal);
    }
    ...
    public void Macos_packaging_path_leak_checks_cover_all_catalogued_first_party_assemblies()
    {
        ...
        var expected = catalog.RootElement.GetProperty("dependencies").EnumerateArray()
            ...
            .Where(file => file.StartsWith("Asura", StringComparison.Ordinal))
    ...
    public void Macos_dotnet_run_assembles_the_required_cef_application_bundle()
    {
        ...
        Assert.Contains("Asura Helper (Renderer)", developmentRunner, StringComparison.Ordinal);
        Assert.Contains("Contents/MacOS/Asura", developmentRunner, StringComparison.Ordinal);
    }
    ...
    public void ProductSourceUsesPanelTerminology()
    {
        ...
        foreach (var file in productFiles)
        {
            ...
            Assert.False(
                ...
                $"{Path.GetRelativePath(RepositoryRoot, file)} uses tmux's pane terminology; Asura calls this a panel.");
        }
    }
    ...
    {
        var applicationRoot = Path.Combine(RepositoryRoot, "src", "Asura.App");
...
, StringComparison.Ordinal) && string.Equals(
        ...
        var status = Assert.Single(
            ...
            element => element.Name == XName.Get("LiveRegionTextBlock", "using:Asura.App.Controls") && string.Equals(
