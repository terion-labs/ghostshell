...
{
    private const string GuestDatabase = "/home/asura/workspace-backend-test.db";
    ...
    public async Task Fresh_sdk_guest_installs_and_executes_database_operations_while_network_is_blocked()
    {
        ...
        Assert.True(File.Exists(Path.Combine(AppContext.BaseDirectory, "runtimes", "osx-arm64", "native",
            "asura-workspace-gateway-darwin-arm64")), "Build and copy the host workspace gateway before running this fixture.");
        Assert.False(File.Exists(GuestDatabase));
        var directory = Directory.CreateTempSubdirectory("asura-backend-sdk-test-");
        ...
        string directory, CancellationToken token)
    {
        foreach (var disposeWorker in new[] { false, true })
        {
            ...
            try
            {
                ...
                var scratch = "/tmp/asura-database-operations/" + operationId;
    ...
    {
        const string guestFile = "/home/asura/workspace-backend-test.duckdb";
    ...
    private sealed class GuestCommands(IWorkspaceIsolationProvider provider, WorkspaceIsolationBinding binding) : IConnectionCommandRuntime
    {
        ...
        {
            DuplexCalls++;
            if (executable.EndsWith("/Asura.Backend", StringComparison.Ordinal))
            {
    ...
    private sealed class WorkspaceBackendFactAttribute : FactAttribute
    {
        public WorkspaceBackendFactAttribute()
        {
            if (!OperatingSystem.IsMacOS()
                || string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("ASURA_TEST_SDK_RUNTIME_ROOT"))
                || string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("ASURA_WORKSPACE_BACKEND_ARCHIVE")))
            {
