...
{
    private readonly string _directory = Path.Combine(Path.GetTempPath(), "asura-backend-package-" + Guid.NewGuid().ToString("N"));
    ...
    public async Task BackendArchiveIsDeterministicAndContainsAnIntegrityManifest(string architecture)
    {
        ...
        var archive = Path.Combine(distribution, $"Asura-workspace-backend-{architecture}.tar.gz");
        ...
        File.SetLastWriteTimeUtc(Path.Combine(payload, "Asura.Backend"), DateTime.UtcNow.AddDays(-30));
        _ = await Package("build", _directory, distribution, payload, sourceDigest, "10.0.111");
        ...
        Assert.Contains("  Asura.Backend\n", manifest, StringComparison.Ordinal);
        Assert.Contains("  Asura.Backend.runtimeconfig.json\n", manifest, StringComparison.Ordinal);
    ...
    public async Task RejectsWrongArchitectureAndRuntime()
    {
        ...
        var payload = CreatePayload();
        var sourceDigest = await RunAsync("source-digest", _directory);
        var executable = Path.Combine(payload, "Asura.Backend");
        ...
        await File.WriteAllTextAsync(Path.Combine(payload, "Asura.Backend.runtimeconfig.json"),
    ...
    public async Task PublicationRequiresSeparateApprovalOfUnchangedLinuxEvidence()
    {
        ...
        string[] names = ["src/Asura.Backend/packages.linux-arm64.lock.json", "licenses/workspace-backend-managed-components.json", "licenses/SMBLIBRARY-SOURCE.json"];
        ...
        {
            schemaVersion = 1,
            format = "asura-workspace-backend-release-legal-v1",
    ...
    public void DesktopShipsOnlyTheDescriptorAndReleasePublishesBackendSeparately()
    {
        ...
        var project = XDocument.Load(Path.Combine(root, "src", "Asura.Desktop", "Asura.Desktop.csproj"));
        var backends = project.Descendants("Content").Where(item =>
            ((string?)item.Attribute("Include"))?.Contains("AsuraWorkspaceBackendDirectory", StringComparison.Ordinal) == true).ToArray();
        ...
        foreach (var architecture in new[] { "arm64", "x64" })
        {
            ...
            Assert.Equal(architecture is "arm64" ? "$(AsuraWorkspaceBackendDirectory)/backend-assets.json"
                : "$(AsuraWorkspaceBackendDirectory)/x64/backend-assets.json", (string?)backend.Attribute("Include"));
        }
        ...
        Assert.Contains("/distribution/Asura-workspace-backend-arm64.tar.gz", workflow, StringComparison.Ordinal);
        Assert.Contains("ASURA_BACKEND_ARCH=x64 ./scripts/build-workspace-backend.sh", workflow, StringComparison.Ordinal);
        Assert.Contains("/distribution/Asura-workspace-backend-x64.tar.gz", workflow, StringComparison.Ordinal);
    }
    ...
    private string CreatePayload(string architecture = "arm64")
    {
        ...
        File.WriteAllBytes(Path.Combine(payload, "Asura.Backend"), header);
        File.WriteAllText(Path.Combine(payload, "Asura.Backend.runtimeconfig.json"),
    ...
    private static Task<string> RunForArchitectureAsync(string architecture, params string[] arguments) =>
        RunPythonAsync(["-c", "import os,runpy,sys; os.environ['ASURA_BACKEND_ARCH']=sys.argv.pop(1); sys.argv=sys.argv[1:]; runpy.run_path(sys.argv[0],run_name='__main__')",
