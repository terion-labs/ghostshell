...
public sealed class ProjectDependencyTests
{
    ...
    public void ActiveVendorProjectsHaveNormalSolutionConfigurationMapping(string projectPath)
    {
        ...
        var project = Assert.Single(LoadProject("Asura.slnx").Descendants("Project"), element =>
            string.Equals((string?)element.Attribute("Path"), projectPath, StringComparison.Ordinal));
        Assert.Empty(project.Elements());
    }

    [Theory]
    [InlineData("src/Asura.Previews/Asura.Previews.csproj", "SharpCompress")]
    [InlineData("src/Asura.Infrastructure/Asura.Infrastructure.csproj", "SSH.NET")]
    [InlineData("src/Asura.Files/Asura.Files.csproj", "SSH.NET")]
    ...
    {
        var project = LoadProject("src/Asura.Core/Asura.Core.csproj");
        Assert.Empty(References(project, "ProjectReference"));
        Assert.Empty(References(project, "PackageReference"));
    }

    [Theory]
    [InlineData("src/Asura.Application/Asura.Application.csproj")]
    [InlineData("src/Asura.Protocol/Asura.Protocol.csproj")]
    public void ApplicationAndProtocolReferenceOnlyCore(string projectPath)
    {
        ...
        Assert.EndsWith("Asura.Core.csproj", reference, StringComparison.Ordinal);
    }
    ...
    public void PresentationDoesNotReferenceConcreteEnginesOrHost()
    {
        var references = References(
            LoadProject("src/Asura.App/Asura.App.csproj"),
            "ProjectReference");
    ...
    {
        var sourceRoot = Path.Combine(RepositoryRoot, "src/Asura.App");
        var files = Directory
        ...
        foreach (var file in files)
        {
            ...
            Assert.DoesNotContain("Asura.Terminal", source, StringComparison.Ordinal);
    ...
    {
        var project = LoadProject("src/Asura.Browser/Asura.Browser.csproj");
        ...
        Assert.Contains(
            ...
            reference => reference.EndsWith(
                "Asura.Application/Asura.Application.csproj",
    ...
    public void InfrastructureDependsOnlyOnApplicationAndCoreProjects()
    {
        var projectReferences = References(
            LoadProject("src/Asura.Infrastructure/Asura.Infrastructure.csproj"),
            "ProjectReference");
        ...
        Assert.Equal(
            ["Asura.Application.csproj", "Asura.Core.csproj"],
    ...
    {
        var projectReferences = References(
                LoadProject("src/Asura.Files/Asura.Files.csproj"),
                "ProjectReference");
        var references = projectReferences
            .Select(reference => Path.GetFileName(
                reference.Replace('\\', Path.DirectorySeparatorChar)))
        ...
        Assert.All(references, reference => Assert.Contains(
            reference,
            ["Asura.Application.csproj", "Asura.Core.csproj"], StringComparer.Ordinal));

        var sourceRoot = Path.Combine(RepositoryRoot, "src/Asura.Files");
        var banned = new[]
        {
            "Avalonia",
            "Ghostty",
    ...
    {
        var references = References(
                LoadProject("src/Asura.Monitoring/Asura.Monitoring.csproj"),
                "ProjectReference")
            .Select(reference => Path.GetFileName(
                reference.Replace('\\', Path.DirectorySeparatorChar)))
            .Order(StringComparer.Ordinal)
        ...
        Assert.Equal(
            new[] { "Asura.Application.csproj", "Asura.Core.csproj" },
            references);

        var project = LoadProject("src/Asura.Monitoring/Asura.Monitoring.csproj");
        Assert.Empty(References(project, "PackageReference"));
        var sourceRoot = Path.Combine(RepositoryRoot, "src/Asura.Monitoring");
    ...
    {
        var projectReferences = References(
                LoadProject("src/Asura.Databases/Asura.Databases.csproj"),
                "ProjectReference");
        var references = projectReferences
            .Select(reference => Path.GetFileName(
                reference.Replace('\\', Path.DirectorySeparatorChar)))
        ...
        var sourceRoot = Path.Combine(RepositoryRoot, "src/Asura.Databases");
        foreach (var file in Directory.EnumerateFiles(
                     sourceRoot,
                     "*.cs",
                     SearchOption.TopDirectoryOnly))
    ...
    {
        var sourceRoot = Path.Combine(RepositoryRoot, "src/Asura.Protocol");
        var banned = new[]
        {
            "Avalonia",
            "Ghostty",
    ...
    {
        var project = LoadProject("src/Asura.ConnectionBackend/Asura.ConnectionBackend.csproj");
        Assert.Empty(References(project, "PackageReference"));
    ...
    {
        var references = References(
            LoadProject("src/Asura.Desktop/Asura.Desktop.csproj"),
            "ProjectReference");
        Assert.Contains(references, reference => reference.EndsWith(
            "Asura.Terminal.csproj",
            StringComparison.Ordinal));
        Assert.Contains(references, reference => reference.EndsWith(
            "Asura.Browser.csproj",
            StringComparison.Ordinal));
        Assert.Contains(references, reference => reference.EndsWith(
            "Asura.SessionHost.csproj",
            StringComparison.Ordinal));
        Assert.Contains(references, reference => reference.EndsWith(
            "Asura.Infrastructure.csproj",
            StringComparison.Ordinal));
        Assert.Contains(references, reference => reference.EndsWith(
            "Asura.Monitoring.csproj",
            StringComparison.Ordinal));
        Assert.Contains(references, reference => reference.EndsWith(
    ...
    public void AvaloniaPackagesShareASecurityPatchedVersion()
    {
        var versions = new[]
            {
                "src/Asura.App/Asura.App.csproj",
                "src/Asura.Browser/Asura.Browser.csproj",
                "src/Asura.Desktop/Asura.Desktop.csproj",
            }
    ...
    {
        var project = LoadProject("src/Asura.Desktop/Asura.Desktop.csproj");
        var reference = Assert.Single(project.Descendants("PackageReference"), element =>
            string.Equals(
                (string?)element.Attribute("Include"),
                "Tmds.DBus.Protocol",
