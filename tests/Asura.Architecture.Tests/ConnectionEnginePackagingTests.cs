...
public sealed class ConnectionEnginePackagingTests
{
    ...
    {
        var project = XDocument.Load(Path.Combine(RepositoryRoot, "src", "Asura.Desktop", "Asura.Desktop.csproj"));
        ...
        var runtime = Assert.Single(content, element => string.Equals((string?)element.Attribute("Include"),
            "$(AsuraWorkspaceRuntimeDirectory)/**/*", StringComparison.Ordinal));
        ...
        var legal = Assert.Single(content, element => string.Equals((string?)element.Attribute("Include"),
            "$(AsuraWorkspaceRuntimeDirectory)/legal/**/*", StringComparison.Ordinal));
        ...
        Assert.Contains("/distribution/Asura-workspace-boot-arm64.zip", workflow, StringComparison.Ordinal);
        Assert.Contains("/distribution/Asura-networking-sources.zip", workflow, StringComparison.Ordinal);
    ...
    public void OnlyTheWorkspaceRuntimeReceivesVirtualizationPrivileges()
    {
        var entitlements = XDocument.Load(Path.Combine(RepositoryRoot,
            "tools", "Asura.Packaging", "MacOS", "WorkspaceRuntime.entitlements"));
        ...
        Assert.Contains("-p:AsuraWorkspaceRuntimeRequired=true", package, StringComparison.Ordinal);
    ...
    public void BuilderLocksSourcesAndProducesTheCompleteMacArm64Closure()
    {
        ...
        Assert.Contains("asura-openvpn-engine", script, StringComparison.Ordinal);
    ...
    public void DesktopPublishSeparatesExecutableCodeFromLegalEvidence()
    {
        ...
        var requiredFiles = project.Descendants("AsuraConnectionEnginesRequiredFile")
            .Select(element => (string?)element.Attribute("Include"))
            .Where(static include => include is not null)
            .ToArray();
        Assert.Contains(
            "$(AsuraConnectionEnginesDirectory)/asura-openvpn-engine",
            requiredFiles,
            StringComparer.Ordinal);
        Assert.Contains(
            "$(AsuraConnectionEnginesDirectory)/OPENVPN-VERSIONS.txt",
            requiredFiles,
            StringComparer.Ordinal);
        Assert.Contains(
            "$(AsuraConnectionEnginesDirectory)/OPENVPN-MPL-2.0.txt",
            requiredFiles,
            StringComparer.Ordinal);
        ...
        Assert.Contains(
            "$(AsuraConnectionEnginesDirectory)/openconnect",
            requiredFiles,
            StringComparer.Ordinal);
        Assert.Contains(
            "$(AsuraConnectionEnginesDirectory)/OPENCONNECT-SOURCE-AND-RELINKING.md",
            requiredFiles,
            StringComparer.Ordinal);
        Assert.Contains(
            "$(AsuraConnectionEnginesDirectory)/sources/openconnect-9.21.tar.gz",
            requiredFiles,
            StringComparer.Ordinal);
    ...
    public void DevelopmentAndReleaseAssemblyBuildAndVerifyTheClosure()
    {
        ...
        Assert.Contains("-p:AsuraConnectionEnginesRequired=true", package, StringComparison.Ordinal);
