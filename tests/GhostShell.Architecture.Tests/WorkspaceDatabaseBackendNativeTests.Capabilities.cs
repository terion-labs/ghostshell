...
public sealed partial class WorkspaceDatabaseBackendNativeTests
{
    private async Task VerifyGuestFileProviderAsync(WorkspaceDatabaseBackend backend, GuestCommands commands, CancellationToken token)
    {
        ...
        var profile = new FileProviderProfile(new("guest-files"), 1, "Guest fixture files", new FileProviderConfiguration.Local("/home/ghostshell"));
        ...
        Assert.Equal("guest-only", await RunShellAsync(commands,
            "test -s /home/ghostshell/guest-backend-content.bin && test ! -e /home/ghostshell/guest-backend-copy.bin && printf guest-only", [], token));
        Assert.False(File.Exists("/home/ghostshell/guest-backend-content.bin"));
    ...
    private async Task ProvisionGuestNetworkFixturesAsync(string assets, WorkspaceIsolationBinding binding, CancellationToken token)
    {
        ...
        await using (var route = ((NetworkConnectionResult<IWorkspacePacketGatewaySession>.Success)opened).Value)
        {
            ...
            var request = new WorkspaceSdkExecRequest(["/bin/sh", "-c", """
                ...
                mkdir -p /run/sshd
                ssh-keygen -A
                systemd-run --unit=ghostshell-test-sshd --collect /usr/sbin/sshd -D -e -f /dev/null -h /etc/ssh/ssh_host_ed25519_key -p 12222 -o ListenAddress=127.0.0.1 -o PasswordAuthentication=no -o UsePAM=yes -o 'Subsystem=sftp internal-sftp'
                systemd-run --unit=ghostshell-test-redis --collect /usr/bin/redis-server --bind 127.0.0.1 --port 16379 --save '' --appendonly no
                systemd-run --unit=ghostshell-test-http --collect /usr/bin/python3 -m http.server 18081 --bind 127.0.0.1 --directory /home/ghostshell
    ...
    private async Task VerifyGuestSftpAsync(WorkspaceDatabaseBackend backend, GuestCommands commands, CancellationToken token)
    {
        ...
        var connection = new ConnectionProfile(new("native-sftp-fixture"), 1, "Guest SFTP fixture",
            new ConnectionEndpoint.Ssh("127.0.0.1", 12222, "ghostshell"),
        ...
        var profile = new FileProviderProfile(new("native-sftp-files"), 1, "Guest SFTP", new FileProviderConfiguration.Sftp(connection.Id, "/home/ghostshell"));
        using (var credentials = new FileWorkspaceHostCredentials(profile, connection, vault, trust, null))
        {
            ...
            Assert.Equal("authorized", await RunShellAsync(commands,
                "umask 077; mkdir -p /home/ghostshell/.ssh; printf 'ssh-rsa %s\\n' \"$1\" > /home/ghostshell/.ssh/authorized_keys; printf authorized",
