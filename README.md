...
- Per-workspace networking: SOCKS5/HTTP/HTTPS proxy, WireGuard, OpenVPN, Cisco AnyConnect (OpenConnect), and Tailscale exit-node connections, saved once and assigned per workspace or as the application default. Each route is a userspace tunnel owned by the app, so different workspaces can sit on different VPNs at the same time without changing host routes; a kill switch holds traffic when a route fails. Inside an isolated workspace the VM's only next hop is that route. See [ADR 0054](./docs/adr/0054-workspace-network-routing.md).
