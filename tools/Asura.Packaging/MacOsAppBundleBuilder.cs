...
public sealed class MacOsAppBundleBuilder
{
    ...
    private const string InfoPlistResource =
        "Asura.Packaging.MacOS.Info.plist.template";
    private const string AppIconFileName = "Asura.icns";
    ...
    private const string ProductVersionPlaceholder = "__ASURA_VERSION__";
    private const string BuildVersionPlaceholder = "__ASURA_BUILD_VERSION__";
    ...
    private const string ProjectLicenseFileName = "ASURA-LICENSE.txt";
        ...
        bool hasSeparateManagedEvidence)
    {
        ...
        if (!OperatingSystem.IsWindows())
        {
            ...
            if ((rootFiles["Asura"].UnixMode & executeBits) == 0)
            {
                throw new InvalidDataException(
                    "The published Asura executable lacks an execute bit.");
            }
        }
    }
        ...
        string relativePath)
    {
        ...
        if (string.Equals(relativePath,
                ...
                StringComparison.Ordinal)
            || string.Equals(relativePath, Path.Combine("runtimes", "linux-x64", "workspace-backend", "backend-assets.json"), StringComparison.Ordinal))
        {
            root = resourcesDirectory;
        }
