...
internal static class NativeTerminalPackageProvenance
{
    ...
    private const string CatalogFormat =
        "asura-native-terminal-component-catalog-v1";
    private const string ReceiptFormat =
        "asura-native-terminal-build-receipt-v1";
        ...
        bool requireExactLibrary)
    {
        ...
        if (RequireInt64(abi, "asuraExtension") != 1)
        {
            throw new InvalidDataException(
                "The native terminal receipt has an incompatible Asura extension ABI.");
        }
        RequireString(
            abi,
            "asuraExtensionExport",
            ExtensionAbiExport);
        ...
        JsonElement catalog, JsonElement receipt, bool requireExactLibrary)
    {
        ...
        RequireString(artifact, "path", "libasura_pty.dylib");
        ...
        var path = Path.Combine(executableDirectory, "libasura_pty.dylib");
        if (requireExactLibrary)
        {
    ...
    public static string ComputeSignatureRemovedSha256(string path)
    {
        ...
        var temporary = Directory.CreateTempSubdirectory("asura-native-identity-");
        try
        {
